...
update_ang_vel_target_from_att_error(const Vector3f &attitude_error_rot_vec_rad, const int32_t pitch_cd_cmd, bool wing_deploy, uint32_t tsld, bool lm)
{
    Vector3f rate_target_ang_vel;

    // Compute the roll angular velocity demand from the roll angle error
    const float angleP_roll = _p_angle_roll.update_p_gain(pitch_cd_cmd, wing_deploy, tsld, lm) * _angle_P_scale.x;
    ...
    const float angleP_pitch = _p_angle_pitch.update_p_gain(pitch_cd_cmd, wing_deploy, tsld, lm) * _angle_P_scale.y;
    ...
    const float angleP_yaw = _p_angle_yaw.update_p_gain(pitch_cd_cmd, wing_deploy, tsld, lm) * _angle_P_scale.z;
