...
// void AC_AttitudeControl_Multi::update_pitch_gain_scheduling(int32_t pitch)
// {
//     // based on pitch angle, we should alter the gains
//     // we can independently alter kp and kd
//     if (pitch < 2000) {
//         set_PD_scale_mult(Vector3f(1.0f, 1.0f, 1.0f));
//     }
//     else if (pitch > 7000) { // I could replace 7000 with a parameter once I figure the parameters out
//         set_PD_scale_mult(Vector3f(rll_atten_fw, pit_atten_fw, yaw_atten_fw))
//     }
//     else {
//         set_PD_scale_mult(Vector3f(1.0f + (rll_atten_fw - 1.0f)*(pitch - 2000)/5000, 1.0f + (pit_atten_fw - 1.0f)*(pitch - 2000)/5000, 1.0f + (yaw_atten_fw - 1.0f)*(pitch - 2000)/5000))
//     }
// }
...
void AC_AttitudeControl_Multi::rate_controller_run(int32_t target_pitch_angle, bool wing_deploy, uint32_t tsld, bool launch_mode)
{
    ...
    _motors.set_roll(get_rate_roll_pid().update_all(_ang_vel_body.x, gyro_latest.x,  _dt, _motors.limit.roll, _pd_scale.x, target_pitch_angle, wing_deploy, tsld, launch_mode) + _actuator_sysid.x);
    _motors.set_roll_ff(get_rate_roll_pid().get_ff());

    _motors.set_pitch(get_rate_pitch_pid().update_all(_ang_vel_body.y, gyro_latest.y,  _dt, _motors.limit.pitch, _pd_scale.y, target_pitch_angle, wing_deploy, tsld, launch_mode) + _actuator_sysid.y);
    _motors.set_pitch_ff(get_rate_pitch_pid().get_ff());

    _motors.set_yaw(get_rate_yaw_pid().update_all(_ang_vel_body.z, gyro_latest.z,  _dt, _motors.limit.yaw, _pd_scale.z, target_pitch_angle, wing_deploy, tsld, launch_mode) + _actuator_sysid.z);
